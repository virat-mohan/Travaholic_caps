...
import { classifyPostBarterApplicant, generateGiftFirstVerificationCode, isPostBarterEnabled } from "@/lib/post-barter";
...
export async function POST(request: Request) {
  if (!(await isPostBarterEnabled())) {
    return NextResponse.json({ error: "Pay With A Post isn't available right now" }, { status: 404 });
  }
