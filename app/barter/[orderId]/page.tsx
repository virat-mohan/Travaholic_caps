...
export default async function BarterOrderPage({ params }: { params: Promise<{ orderId: string }> }) {
  // Gate first, before ever touching the DB — a disabled feature must never
  // leak whether a given order id even exists.
  if (!(await isPostBarterEnabled())) notFound();
    ...
    <main className="mx-auto w-full max-w-[600px] px-6 pt-32 pb-24 md:px-12 md:pt-40">
      ...
      {isGiftFirst ? (
        ...
        <div className="mt-8 border-2 border-ink bg-surface-alt p-6">
          <p className="font-sans text-body-s font-bold uppercase text-ink">Your Good Vibes Came Through</p>
          ...
          >
            Spread More Good Vibes — Shop Another Chapter
          </Link>
        </div>
      ) : (
        <>
