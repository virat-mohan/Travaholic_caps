...
/** Whether the whole "Pay With A Post" feature is switched on — every entry point (checkout UI, order creation, /barter/* routes) gates on this. */
export async function isPostBarterEnabled(): Promise<boolean> {
  const value = await getSetting("POST_BARTER_ENABLED");
  return value === "true";
}
...
/** Start of "today" in IST, matching how the rest of the admin reasons about a calendar day. */
...
function codeForWindow(handle: string, windowStart: number): string {
  const h = createHmac("sha256", verificationSecret())
    ...
    .digest("hex");
  return `TRAV-${h.slice(0, 6).toUpperCase()}`;
}
...
// Rotated onto the sharer's first name (e.g. "ANUN" -> "ANUNTRAVAHOLIC") so
// the code reads like something worth sharing rather than a random string.
// Picked randomly per mint (not hashed) since a nicer-sounding code is worth
// more than the same person always landing on the same word.
const THEME_WORDS = [
  "WANDERLUST",
  "HORIZON",
  "OPENROAD",
...
/** Mints a shareable coupon code for this barter order, reusing the exact checkout coupon engine so a friend's redemption is a completely ordinary coupon redemption. Full price for the friend — see discount_value below — this is attribution, not a discount mechanic. */
...
export type PostBarterOrderPayload = {
  ...
  /** Required (server-enforced) whenever the order actually resolves to gift_first — explicit acceptance of the post-after-delivery condition. Meaningless for sell_first, where nothing ships before a post exists anyway. */
 ...
 *
 * Refuses outright (throws) when POST_BARTER_ENABLED isn't "true" — the
 * gating point that keeps the whole feature inert while switched off, even
 * if some other bug lets a client reach this function directly.
 */
export async function createPostBarterOrder(payload: PostBarterOrderPayload) {
  ...
  // post-after-delivery term. Never silently downgrade this one — a shopper
  // who didn't see/accept the term must be told plainly, not quietly
  // re-routed into sell_first.
  ...
  await Promise.allSettled([
    ...
    // Same WhatsApp confirmation every other payment path sends — a barter
    // order shouldn't be the one path that only ever emails the customer.
 ...
 * WhatsApp-manual order path (see app/api/orders/route.ts) must never be
 * able to fake progress toward a free shipment. Also excludes redemptions
 * that match the barterer's own phone/email — buying from themselves with
 * their own code must never count toward their own qualification, and never
 * triggers a progress email either.
 *
 * A no-op (returns immediately) when POST_BARTER_ENABLED isn't "true" —
 * disabling the feature must stop existing sell_first orders from silently
 * continuing to qualify and ship in the background.
 */
export async function maybeQualifyBarterOrderForCoupon(
  ...
  if (!(await isPostBarterEnabled())) return;
  const supabase = getSupabaseServerClient();
