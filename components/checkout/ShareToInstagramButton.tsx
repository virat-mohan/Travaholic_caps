...
const LOGO_IMAGE = "/images/brand/travaholic-logo-mono-white.png";
const TAGLINE = "STORIES YOU CAN WEAR";
 ...
 * named "Manrope" or "Archivo Black" — it generates a scoped local family
 * name and exposes it only via the --font-manrope / --font-archivo-black CSS
 * variables. Canvas text drawn with the literal Google Font name therefore
 * silently falls back to the system font. Reading the real generated names
 * off :root at draw time is what actually gets the brand fonts to render.
 */
function getSiteFonts() {
  const styles = getComputedStyle(document.documentElement);
  const headline = styles.getPropertyValue("--font-archivo-black").trim() || "sans-serif";
  const body = styles.getPropertyValue("--font-manrope").trim() || "sans-serif";
  return { headline: `${headline}, sans-serif`, body: `${body}, sans-serif` };
}

async function ensureFontsReady(headlineFont: string, bodyFont: string) {
  try {
    await Promise.all([
      document.fonts.load(`400 96px ${headlineFont}`),
 ...
 * (product shot, brand logo, tagline, and the code) rather than a sheet of
 * instructions. Deliberately carries NO "post this / tag us" meta-
 * instructions — those belong on the page around the button, since a genuine
 * Instagram post should never read like a how-to. `heroImageUrl` and
 * `productName` are picked per-order server-side (see
 * lib/share-card-pool.ts) so different barterers' posts feature different
...
export async function buildShareCard(
  ...
  const { headline: HEADLINE_FONT, body: BODY_FONT } = getSiteFonts();

  ctx.fillStyle = "#101820";
  ctx.fillRect(0, 0, CANVAS_W, CANVAS_H);

  const TOP_BAND = 260; // solid ink band — logo lives here, never over the photo
  const [hero, logo] = await Promise.all([
    loadImage(heroImageUrl),
    loadImage(LOGO_IMAGE),
    ensureFontsReady(HEADLINE_FONT, BODY_FONT),
  ]);
  ...
  bottomGrad.addColorStop(0, "rgba(16,24,32,0)");
  bottomGrad.addColorStop(0.4, "rgba(16,24,32,0.92)");
  bottomGrad.addColorStop(1, "rgba(16,24,32,0.98)");
  ctx.fillStyle = bottomGrad;
  ctx.fillRect(0, CANVAS_H - 560, CANVAS_W, 560);

  ctx.textAlign = "center";

  // The site's own tagline instead of a generic mood line.
...
export function ShareToInstagramButton({
  ...
  const caption = `Stories you can wear 🧢 Shop ${siteDomain} and use my code ${couponCode} at checkout — tag ${instagramHandle} as collaborator when you post.`;
  ...
  async function share() {
    ...
    if (blob) {
      const file = new File([blob], "travaholic-pay-with-a-post.png", { type: "image/png" });
      ...
      a.href = url;
      a.download = "travaholic-pay-with-a-post.png";
      a.click();
